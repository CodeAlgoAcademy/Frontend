...
const unitsSlice = createSlice({
   ...
   reducers: {
      ...
      displayUnitsAndGradesBasedOnLevels: (state: IUnitsSlice) => {
         ...
         state.addUnit.units = units.reduce((acc: any, currentUnit: any) => {
            if (!acc.find((accUnit: any) => accUnit.id === currentUnit.id)) {
               const unit: any = availableUnits.find((availableUnit) => availableUnit.id == currentUnit.id);
               acc.push(unit);
            }
            return acc;
         }, []);
      ...
      updateUnits: (state: IUnitsSlice, action: PayloadAction<{ id: string; type: string; value?: string }>) => {
      ...
      updateGrades: (state: IUnitsSlice, action: PayloadAction<{ value: string; type: string }>) => {
         if (action.payload.type === "remove") {
            state.addUnit.chosenGrades = state.addUnit.chosenGrades.filter((grade) => grade !== action.payload.value);
      ...
      rearrangeUnits: (state: IUnitsSlice) => {
         ...
         state.addUnit.units
            ...
            .forEach((unit) => {
               ...
               const levelsWithUnit = availableLevels.filter((level) => level.unitsId.includes(`${unit.id}`));
               ...
               let grades: string[] = levelsGrade.filter((grade) => state.addUnit.chosenGrades.includes(grade));

               unitObject.grades = grades.map((grade) => grade);

               // check how many levels it exists and make requests for each level
               const level = availableLevels.find((level) => level.unitsId.includes(`${unit.id}`) && state.addUnit.levels === level.title);
            ...
            .forEach((unit) => {
               if (unit.startDate && unit.endDate) {
                  ...
                  if (start_date === today_date && !unit.isCurrent) {
                     errors.push(`${unit.title} unit start date should be a future date since it is upcoming`);
                  }
                  ...
                  if (end_date <= today_date) {
                     errors.push(`${unit.title} unit end date should be after it's start date and after today's date`);
                  }
