   ...
   return (
      <div className='h-5 flex items-center'>
         ...
         <span className='rounded-r-xl bg-[#ECEDF3] h-full w-full progress-indicator-bar relative'>
            <span className={`rounded-r-xl h-full top-0 left-0 pr-[10px] text-right text-sm text-white block ${color === "green" ? "green-progress-bar" : "red-progress-bar"}`} style={progressBarStyle}>{percentage}%</span>
         </span>
      </div>
   );
};
