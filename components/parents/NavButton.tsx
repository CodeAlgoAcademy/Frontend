...
const NavButton = ({image,title,notification,name,isIcon}: Props) => {
   ...
   return (
      <Link href={`/parents/${name}`}>
         <button className='py-[14px] px-7 flex relative items-center gap-5 w-full hover:bg-slate-50 rounded-3xl' style={{backgroundColor: active ? "#2073fa" : "",padding: !isIcon ? "14px 28px" : "12px",borderRadius: !isIcon ? "24px" : "6px"}} onMouseEnter={() => setDetailsDisplay(() => true)} onMouseLeave={() => setDetailsDisplay(() => false)}>
