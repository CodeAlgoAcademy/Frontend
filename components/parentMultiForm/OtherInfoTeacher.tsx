...
export default function OtherInfoTeacher() {
  ...
  const { schoolName, username } = useSelector(
  ...
  return (
    <div>
      <h1 className='font-bold text-[32px]'>Create Your Account</h1>
        ...
        <input type="text" className='block w-full h-[2.5rem] rounded-xl px-4 py-2 focus:outline-0 mt-3' 
          placeholder='Enter School Name' value={schoolName} required
      ...
      <label className='block text-xl font-semibold mt-6'>Username</label>
        <input type="text" value={username} className='block w-full h-[2.5rem] rounded-xl px-4 py-2 focus:outline-0 mt-3' placeholder='Enter School Name' 
        onChange={(e) => {
