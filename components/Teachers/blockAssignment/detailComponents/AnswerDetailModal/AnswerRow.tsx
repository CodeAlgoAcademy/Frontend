...
=======
  const isLineCoding = ans.question_source === "activity" || ans.question_source === "compiler";
>>>>>>> bb1ee5e (line game progress update)
  const selectedOption = ans.student_answer?.selected_option ?? ans.student_answer?.order?.join(", ");

  return (
    <div className={`grid gap-x-4 px-6 py-5 items-start hover:bg-slate-50/60 transition-colors ${isLineCoding ? "grid-cols-[1fr_120px_150px_140px]" : "grid-cols-[1fr_120px_150px_140px]"}`}>
      <div>
        ...
        <div className="flex flex-col gap-2 pt-0.5 col-span-3">
          {ans.code && (
            <div className="bg-slate-900 rounded-lg p-3 text-xs">
              <div className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider mb-1.5">Code</div>
              <pre className="text-green-400 font-mono whitespace-pre-wrap break-all leading-relaxed">{ans.code}</pre>
            </div>
          )}
            ...
            <div className="flex gap-2">
              {ans.output && (
                <div className="flex-1 bg-slate-800 rounded-lg p-3 text-xs">
                  ...
                  <pre className="text-slate-200 font-mono whitespace-pre-wrap break-all">{ans.output}</pre>
                </div>
              )}
              {ans.errors && (
                <div className="flex-1 bg-red-950 rounded-lg p-3 text-xs">
                  <div className="text-[10px] font-semibold text-red-400 uppercase tracking-wider mb-1">Errors</div>
                  <pre className="text-red-300 font-mono whitespace-pre-wrap break-all">{ans.errors}</pre>
                </div>
              )}
            </div>
          )}
          {ans.ai_rating != null && (
            <div className="flex items-center gap-2 text-xs">
              ...
              <span className={`font-bold ${ans.ai_rating >= 7 ? "text-green-600" : ans.ai_rating >= 4 ? "text-yellow-600" : "text-red-600"}`}>
                {ans.ai_rating}/10
              </span>
              {ans.ai_summary && <span className="text-slate-500">— {ans.ai_summary}</span>}
            </div>
          )}
        </div>
      ) : (
