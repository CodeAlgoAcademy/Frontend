          ...
          <div>
            ...
            <div className="mt-12 flex flex-col gap-4">
              ...
              <div className="px-4 py-2 flex items-center justify-between w-full relative rounded-lg z-top h-[46px] bg-gray-100 drop-shadow-md">
                ...
                <span className="opacity-60 text-3xl cursor-pointer" onClick={ () => {
                  ...
                  <div className="grid grid-cols-1 items-center gap-1 w-[90%] absolute top-10 left-0 rounded-lg overflow-y-auto h-32 bg-gray-100 drop-shadow-md small-scroll-thumb">
                    {
                      ["CTSA", "Kansas", "New York", "Missouri"].map((skill) => (
                        <span key={ skill } className="opacity-50 text-sm text-center py-2 cursor-pointer hover:bg-gray-300 font-semibold" onClick={ () => {
                          setSkillType((prev) => skill)
                          setPickerDisplay((prev) => { return { grade: false, skill: false } })
                        } }>
                          { skill }
                        </span>)
                      )
              ...
              <div className="px-4 py-2 flex items-center justify-between w-full rounded-lg h-[46px] relative bg-gray-100 drop-shadow-md">
                ...
                <span className="opacity-60 text-3xl cursor-pointer" onClick={ () => { setPickerDisplay((prev) => { return { skill: false, grade: !prev.grade } }) } }>
                  <RiArrowDropDownLine />
                </span>
                {
                  pickerDisplay.grade &&
                  <div className="grid grid-cols-1 items-center gap-1 w-[90%] absolute top-10 left-0 rounded-lg overflow-y-auto h-48 bg-gray-100 drop-shadow-md small-scroll-thumb">
                    {
                      ["Kindergarden", "Grade 1", "Grade 2", "Grade 3", "Grade 4", "Grade 5", "Grade 6", "Grade 7", "Grade 8"].map((grade) => (
                        <span key={ grade } className="opacity-50 text-sm text-center py-2 cursor-pointer hover:bg-gray-300 font-semibold" onClick={ () => {
                          setGrade((prev) => grade)
                          setPickerDisplay((prev) => { return { grade: false, skill: false } })
                        } }>
                          { grade }
