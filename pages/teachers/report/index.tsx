...
>>>>>>> bb1ee5e (line game progress update)
};
...
=======
   ...
   return (
      <div className="flex flex-col gap-1.5">
         ...
         >
            <span style={{ background: cfg.dot }} className="h-2 w-2 flex-shrink-0 rounded-full" />
            {cfg.label}
         </span>
         <div className="flex w-28 items-center gap-2">
            <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-slate-100">
               <div style={{ width: `${cfg.pct}%`, background: cfg.dot }} className="h-full rounded-full transition-all duration-700" />
>>>>>>> bb1ee5e (line game progress update)
            </div>
            <span className="w-7 text-right font-mono text-[10px] text-slate-400">{cfg.pct}%</span>
         </div>
      </div>
   );
}
...
=======
   const cfg = PERSISTENCE_CONFIG[band?.toLowerCase()];
   if (!cfg || !band) return <span className="font-medium text-slate-300">—</span>;
   return (
      <span
         style={{ color: cfg.color, background: cfg.bg, border: `1px solid ${cfg.border}` }}
   ...
   useEffect(() => {
      if (classId) {
         ...
         dispatch(fetchLineDiagnosticSummary(classId))
            .unwrap()
            .then((res) => console.log("[LineDiagnostics] API response:", res))
            .catch((err) => console.error("[LineDiagnostics] API failed:", err));
      }
   }, [classId, dispatch]);

   const activeData = useMemo(() => {
      const sourceData = activeTab === 'block' ? diagnosticSummary : lineDiagnosticSummary;
      return sourceData.filter(student => student.last_completed_unit_level !== null);
   }, [activeTab, diagnosticSummary, lineDiagnosticSummary]);

   const total = activeData.length;
   const assessed = activeData.filter((s) => !!s.mastery_band).length;
   const pct = total > 0 ? Math.round((assessed / total) * 100) : 0;

   const tabs: { key: TabType; label: string }[] = [
      { key: "block", label: "Block Coding" },
      ...
      <TeacherLayout>
         <div className="min-h-screen bg-[#f0f4f8] p-4 sm:p-8">
            <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
               <div>
                  <p className="mb-1 text-[11px] font-black uppercase tracking-[0.2em] text-indigo-500">Class Report</p>
                  <h1 className="text-2xl font-black text-slate-800 sm:text-3xl">Diagnostic Class Results</h1>
                  <p className="mt-1 text-sm text-slate-500">
                     {activeTab === "block" ? "Visual Scripting & Block Coding" : "Line Coding & Python"} · Progress Analysis
                  </p>
               </div>
               <div className="flex flex-shrink-0 gap-3">
                  <StatCard value={total} label="Students" />
                  <StatCard value={assessed} label="Assessed" accent={activeTab === "block" ? "text-indigo-600" : "text-purple-600"} />
...
>>>>>>> bb1ee5e (line game progress update)
            ...
            <div className="mb-4 flex flex-wrap items-center gap-3">
               ...
               <div className="flex flex-wrap items-center gap-x-5 gap-y-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 shadow-sm">
                  <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Mastery Scale</span>
                  {MASTERY_ORDER.map((key) => {
                     const cfg = MASTERY_CONFIG[key];
                     return (
                        <span key={key} className="flex items-center gap-1.5 text-[11px] font-semibold" style={{ color: cfg.color }}>
                           <span style={{ background: cfg.dot }} className="h-2 w-2 rounded-full" />
                           {cfg.label}
                        </span>
                     );
                  })}
               ...
               <div className="overflow-x-auto">
                  <table className="w-full min-w-[740px] border-collapse text-left">
                     ...
                     <tbody>
                        ...
                        {!isLoading && activeData.map((student, i) => (
                           <tr
                              key={student.student_id}
                              className={`border-b border-slate-50 transition-colors
                                            ${activeTab === "block" ? "hover:bg-indigo-50/30" : "hover:bg-purple-50/30"}
                                            ${i % 2 !== 0 ? "bg-slate-50/40" : ""}`}
                           >
                              <td className="px-5 py-4">
                                 <div className="flex items-center gap-3">
                                    <div>
                                       <p className="text-sm font-bold leading-snug text-mainColor">{student.student_name}</p>
                                       {student.flags.length > 0 ? (
                                          ...
                                          <p className="mt-0.5 text-[10px] text-slate-500">Diagnostic pending</p>
                                       )}
                                    </div>
                                 </div>
                              </td>
...
{activeData.length === 0 && !isLoading && (
                           <tr>
                              <td colSpan={6} className="text-center py-20 text-slate-400">
                                 <p className="text-3xl mb-2">{activeTab === 'block' ? '📋' : '🐍'}</p>
                                 <p className="font-semibold">
                                    {activeTab === 'block'
                                       ? 'No Block Coding diagnostics found for this class.'
                                       : 'No Python (Line) diagnostics recorded yet. This section tracks students age 14+.'}
                                 </p>
                              </td>
                           </tr>
                        )}
               ...
               {total > 0 && (
                  <div className="flex items-center justify-between gap-4 border-t border-slate-100 px-6 py-3">
                     <p className="text-[11px] text-slate-400">
                        <span className="font-bold text-slate-600">{assessed}</span> of <span className="font-bold text-slate-600">{total}</span>{" "}
                        students assessed
                     </p>
                     <div className="flex items-center gap-2">
                        <div className="h-1.5 w-36 overflow-hidden rounded-full bg-slate-100">
                           <div
                              style={{ width: `${pct}%` }}
                              className={`h-full rounded-full transition-all duration-700 ${
                                 activeTab === "block" ? "bg-indigo-500" : "bg-purple-500"
                              }`}
                           />
                        </div>
                        <span className={`text-[11px] font-bold ${activeTab === "block" ? "text-indigo-600" : "text-purple-600"}`}>{pct}%</span>
                     </div>
                  </div>
               )}
            </div>
         ...
         {selectedStudentId && activeTab === "block" && (
            <DiagnosticDetailModal studentId={selectedStudentId} onClose={() => setSelectedStudentId(null)} />
         )}
         {selectedStudentId && activeTab === "python" && (
            <LineDiagnosticModal studentId={selectedStudentId} onClose={() => setSelectedStudentId(null)} />
         )}
      </TeacherLayout>
   );
}
