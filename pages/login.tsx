        ...
        <div className="bg-white w-[95vw] max-w-[600px] mx-auto rounded-md p-[40px] md:p-[50px] shadow-md">
          ...
          <div className="inline w- mx-auto">
            <span className="flex items-center gap-8 mt-6 justify-center relative">
              { recaptchaLoading ?
                <div className="relative w-3 h-3"><div className="spinner center">
                ...
                <label className="checkbox_container block relative cursor-pointer text-[20px] select-none">
                  <input checked={ recaptchaVerified } type="checkbox" className='absolute opacity-0 cursor-pointer h-0 w-0' onChange={ (e) => {
                    if (e.target.checked) {
                      handleReCaptchaVerify()
                      setRecaptchaLoading(true)
                    }
                  } } />
                  <div className="box_checkmark checkmark relative left-0 h-[1.3em] w-[1.3em] bg-[#606062] rounded-[5px] shadow-md" style={ { top: "-12px" } }></div>
                </label>
              }
              { notification === "Verification Successfull" && <p className='text-green-500 text-sm font-semibold text-center'>{ notification }</p> }
              { notification === "Verification Failed" || notification === "Error submitting verification data" && <p className='text-red-500 text-sm font-semibold text-center'>{ notification }</p> }
              { notification === "" && <p className='text-sm font-semibold text-center'>Click to verify you're not a robot.</p> }
            </span>
          </div>
        </div>
      </section>
