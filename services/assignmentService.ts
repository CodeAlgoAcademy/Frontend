...
export const addNewAssignments: any = createAsyncThunk(
   ...
   async (
      ...
      if (!is_current && !is_finished && new Date(start_date) > new Date()) {
         errors.push("This unit is scheduled for a later date, kindly update to current before adding assignments");
      }
      ...
      try {
         if (errors.length === 0) {
            const { data } = await http.post(`/academics/curriculums/units/${id}/assignment/`, mainAssignment, {
               headers: {
...
export const updateAssignment: any = createAsyncThunk(
   ...
   async (
      ...
      try {
         if (errors.length === 0) {
            const { data } = await http.put(`/academics/curriculums/units/${id}/assignment/${assignmentId}`, mainAssignment, {
               headers: {
                  Authorization: `Bearer ${getAccessToken()}`,
               },
            });
